...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$s='dist/faceit-rpc-win'; if(Test-Path $s){Remove-Item $s -Recurse}; New-Item -ItemType Directory $s | Out-Null; Copy-Item 'bin/faceit-rpc.exe' $s; Copy-Item 'dist/faceit-rpc.xpi' $s; Copy-Item 'art.txt' $s; New-Item -ItemType Directory (Join-Path $s 'extensions') | Out-Null; Copy-Item 'extensions/chromium' (Join-Path $s 'extensions') -Recurse; Copy-Item 'start_daemon.bat' $s; Copy-Item 'README.md' $s; $z='dist/faceit-rpc-win.zip'; if(Test-Path $z){Remove-Item $z}; Compress-Archive -Path $s -DestinationPath $z -Force; Remove-Item $s -Recurse; Write-Host ('Bundle ready: ' + $z)"
echo.
